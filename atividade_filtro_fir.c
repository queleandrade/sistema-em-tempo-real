...
static int coeff[N_FILTRO] = {-3, 12, 17, 12, -3};
...
void app_main() {
  ...
  cb_init (&cbuf, N_FILTRO);

  while (true) {
        if (cb_get_filled(&cbuf) >= N_FILTRO ) {
            saida_i = 0;
            for (int i=0; i<N_FILTRO; i++)
               saida_i += coeff[i] * cbuf.buffer[(cbuf.tail+i)%N_FILTRO];
            saida_f = ((float)saida_i)/35;
      
            cb_pop(&cbuf, &saida_i,1);
